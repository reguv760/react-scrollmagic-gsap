import React from "react";

function Content() {
  return (
    <div className='content'>
      <div className='content-wrapper'>
        <div className='content-images'>
          <div className='content-image-wrapper one'>
            <img
              src='https://images.unsplash.com/photo-1550422086-b9589ec2d371?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=675&q=80'
              alt='W'
            />
            <h4>W</h4>
          </div>
          <div className='content-image-wrapper two'>
            <img
              src='https://images.unsplash.com/photo-1550433251-d1a8ff40ff05?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=634&q=80'
              alt='E'
            />
            <h4>E</h4>
          </div>
          <div className='content-image-wrapper three'>
            <img
              src='https://images.unsplash.com/photo-1550364387-ffbad4f8e9b2?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=634&q=80'
              alt='S'
            />
            <h4>S</h4>
          </div>
          <div className='content-image-wrapper four'>
            <img
              src='https://images.unsplash.com/photo-1550183203-8d3ca82400c0?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=634&q=80'
              alt='T'
            />
            <h4>T</h4>
          </div>
        </div>
        <p className='text'>
          Lorem ipsum dolor amet schlitz sriracha brooklyn prism craft beer air
          plant PBR&B swag jianbing gluten-free XOXO echo park snackwave
          keffiyeh microdosing. Vape mumblecore squid pork belly keytar franzen
          fingerstache truffaut. Single-origin coffee forage helvetica
          chartreuse, kinfolk kogi tumblr cornhole offal truffaut VHS pinterest.
          Health goth locavore etsy four dollar toast irony man bun hella
          waistcoat VHS taiyaki artisan umami synth ramps. Roof party plaid
          snackwave normcore microdosing poke gochujang, butcher photo booth
          franzen mixtape ennui selvage.
        </p>
        {/* <p className='text'>
          Hammock XOXO gastropub woke stumptown. Wolf photo booth lo-fi
          chicharrones tote bag. Actually selfies affogato cred. 90's hot
          chicken 8-bit post-ironic. Bicycle rights blog brooklyn four dollar
          toast microdosing.
        </p>
        <p className='text'>
          Hammock XOXO gastropub woke stumptown. Wolf photo booth lo-fi
          chicharrones tote bag. Actually selfies affogato cred. 90's hot
          chicken 8-bit post-ironic. Bicycle rights blog brooklyn four dollar
          toast microdosing.
        </p>
        <p className='text'>
          Hammock XOXO gastropub woke stumptown. Wolf photo booth lo-fi
          chicharrones tote bag. Actually selfies affogato cred. 90's hot
          chicken 8-bit post-ironic. Bicycle rights blog brooklyn four dollar
          toast microdosing.
        </p> */}
      </div>
    </div>
  );
}

export default Content;
